...
class LossComputer:
    def __init__(self, criterion, is_robust, dataset, alpha=None, gamma=0.1, adj=None, min_var_weight=0, step_size=0.01, normalize_loss=False, btl=False):
        ...
        self.group_counts = dataset.group_counts().to(get_device())
        self.group_frac = self.group_counts/self.group_counts.sum()
        self.group_str = dataset.group_str

        if adj is not None:
            self.adj = torch.from_numpy(adj).float().to(get_device())
        else:
            self.adj = torch.zeros(self.n_groups).float().to(get_device())

        if is_robust:
            assert alpha, 'alpha must be specified'

        # quantities maintained throughout training
        self.adv_probs = torch.ones(self.n_groups).to(get_device())/self.n_groups
        self.exp_avg_loss = torch.zeros(self.n_groups).to(get_device())
        self.exp_avg_initialized = torch.zeros(self.n_groups).byte().to(get_device())
    ...
    def compute_group_avg(self, losses, group_idx):
        ...
        group_map = (group_idx == torch.arange(self.n_groups).unsqueeze(1).long().to(get_device())).float()
    ...
    def reset_stats(self):
        self.processed_data_counts = torch.zeros(self.n_groups).to(get_device())
        self.update_data_counts = torch.zeros(self.n_groups).to(get_device())
        self.update_batch_counts = torch.zeros(self.n_groups).to(get_device())
        self.avg_group_loss = torch.zeros(self.n_groups).to(get_device())
        self.avg_group_acc = torch.zeros(self.n_groups).to(get_device())
