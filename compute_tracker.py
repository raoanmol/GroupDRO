...
class ComputeTracker:
    ...
    def end_training(self) -> None:
        ...
        if self._total_energy_joules > 0:
            ...
            summary['total_energy_kwh'] = round(
                self._total_energy_joules / 3.6e6, 6)

        self._training_summary = summary

    def start_phase(self, epoch: int, phase: str,
                    num_samples: int, num_batches: int) -> None:
        if self.device.type == 'cuda':
    ...
    def end_phase(self, epoch: int, phase: str) -> None:
        ...
        if self._nvml_handle is not None and 'power_start_mw' in timer:
            try:
                power_end_mw = pynvml.nvmlDeviceGetPowerUsage(
                    self._nvml_handle)
    ...
    def _get_or_create_epoch(self, epoch: int) -> Dict:
        for entry in self._epoch_metrics:
            if entry['epoch'] == epoch:
                return entry
        entry: Dict[str, Any] = {'epoch': epoch, 'phases': {}, 'total_wall_clock_s': 0.0}
